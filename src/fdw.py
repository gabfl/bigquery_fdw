...
class ConstantForeignDataWrapper(ForeignDataWrapper):
    ...
    def setOptions(self, options):
        ...
        try:
            ...
            log_to_postgres(
                "You must specify these options when creating the FDW: fdw_dataset, fdw_table",
                ERROR,
            )
    ...
    def buildColumnList(self, columns, usage="SELECT"):
        ...
        if columns:  # If we have columns
            for column in columns:
                if (
                    column == self.countPseudoColumn
                ):  # Pseudo column to count grouped rows
                    if usage == "SELECT":
                        clause += (
                            "count(*) " + self.addColumnAlias(column, useAliases) + ", "
                        )
                    ...
                    clause += (
                        column
                        + " "
                        + self.addColumnAlias(columnOriginalName, useAliases)
                        + ", "
                    )
    ...
    def castColumn(self, column, columnOriginalName, dataType):
        ...
        if (
            self.castingRules and columnOriginalName in self.castingRules
        ):  # If we have casting rule for this column
            # Get desired casting
            ...
            rule = [
                conversionRule
                for conversionRule in self.conversionRules
                if conversionRule.bq_standard_from == dataType.upper()
            ]
        ...
        if quals:
            ...
            for qual in quals:
                if qual.field_name == self.partitionPseudoColumn:
                    clause += (
                        "_PARTITIONTIME " + str(self.getOperator(qual.operator)) + " ?"
                    )
                    ...
                    parameters.append(
                        self.setParameter(qual.field_name, "TIMESTAMP", value)
                    )
                else:
                    clause += (
                        str(qual.field_name)
                    ...
                    parameters.append(
                        self.setParameter(
                            qual.field_name,
                            self.getBigQueryDatatype(qual.field_name),
                            qual.value,
                        )
                    )
    ...
    def getBigQueryDatatype(self, column, dialect="standard"):
        ...
        for datatype in self.datatypes:  # For each known data types
            if (
                datatype.postgres == pgDatatype
            ):  # If the PostgreSQL data type matches the known data type
